...
// Rreturn array with ascending or descending indexes incremented by "step" amount starting from desired number. 
// Used here to get every third day, because of how the hourly-weather fetch result is formatted 
...
const HourlyWeather = () => {
  ...
  // Display weather for every third hour
    ...
    <div className={`my-3 w-full h-32 min-h-[8rem] bg-slate-800 rounded-xl backdrop-blur-lg shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] bg-opacity-20 transition-all  duration-500 ${firsRenderContextObject?.firstRender ? "opacity-0" : "opacity-100"} flex justify-center items-center`}>
      {indexes.map(index => {
        ...
        return (
          <HourWindow
            key={index}
            icon={hourlyWeather?.iconNumber[index]}
            temperature={hourlyWeather?.temperature[index]}
            time={(new Date(hourlyWeather?.time[index] * 1000)).getHours()}
          />
        )
      })}
    </div>
