...
import { cityInformationObject, createNewCityObject } from "@/context/cityContext";
...
const CitySearch = () => {
  ...
  const [fetchedCity, setFetchedCity] = useState<cityInformationObject | null>(null);
  ...
  const handleInputCity = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // If the input is lesser than two characters, set fetched city to null and adjust font
  ...
  return (
    <div className="w-full h-20">
      <form className=" w-full h-full relative inline-block from-neutral-900" action="none" placeholder="Test" onSubmit={(e) => { handleSubmit(e) }}>
        ...
        <div className={`text-gray-200 text-lg ${transitionContextObject?.transition}-animation`}>
          {cityContextObject?.city?.country ?? ''}
        </div>
      </form>
    </div>
  );
