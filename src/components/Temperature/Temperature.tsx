...
const Temperature = () => {
    ...
    return ( 
            <div className={`${transitionContextObject?.transition}-animation min-h-[12rem] h-[16rem] flex flex-col justify-center items-center w-full mt-[-26px]  text-white`}>
                <div className="text-8xl font-thin relative mt-2">{temperatrue ? `${Math.round(temperatrue)}`: ''}<span className="absolute mt-2 text-5xl w-0 inline-block">{temperatrue?'º':''}</span></div>
